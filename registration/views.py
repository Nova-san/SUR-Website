...
class EventsListView(TemplateView):
    """(Unused?) Dashboard events list view."""
...
def export_xlsx(request):
    ...
    if request.GET and form.is_valid():
        ...
        if export_type == "google":
            sheet_id = '1ImyS95ef4DPFWk0Nk9A9mhP64bN6savAcYsc01QGpIc'
            export_runners_to_google_sheet(qs, sheet_id)
            messages.success(request, "Exported to Google Sheet successfully.")
            return redirect('registration:export_xlsx')
        ...
        for row_num, runner in enumerate(qs, start=4):
            worksheet.write(row_num, 0, runner.name, bold_name_format)
...
def runners_by_event(request):
    ...
    if request.GET and form.is_valid():
        ...
        runner_list = runner_list.select_related('distance').order_by('name')
...
def export_runners_to_google_sheet(queryset, sheet_id):
    scope = [
        "https://spreadsheets.google.com/feeds",
        "https://www.googleapis.com/auth/drive"
    ]

    key_path = os.path.join(settings.BASE_DIR, "credentials", "sheets-key.json")
    ...
    # Write headers
    sheet.clear()
    sheet.append_row(["Name", "Email", "Distance", "Age", "Gender", "Verified?"])
    ...
    for runner in queryset:
        sheet.append_row([
            ...
            "Yes" if runner.is_verified else "No"
        ])